...
class AddRelatedCrisisAttentionView extends StatefulWidget {
  final int formId;

  const AddRelatedCrisisAttentionView({Key? key,required this.formId}) : super(key: key);
  @override
  _AddRelatedCrisisAttentionViewState createState() => _AddRelatedCrisisAttentionViewState();
}

class _AddRelatedCrisisAttentionViewState extends State<AddRelatedCrisisAttentionView> {
  ...
  Future<List<FormModel>> _searchForm(String text) async {
    ...
    List<FormModel>? forms =  await CrisisAttentionService().searchRelatedForm(delegate: text, fatherId: widget.formId);
  ...
  Future<void> _addToRelatedCases(int formId) async {
    ...
    bool added = await CrisisAttentionService().addToRelatedForms(fatherId: widget.formId,childId: formId);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text("Agregar Caso", style: TextStyle(color: Color(0xff1f0757)),),
      ),
      backgroundColor: Colors.white,
      // body: SafeArea(
      ...
      //     placeHolder: Center(child: Text("Buscar Caso", style: TextStyle(color: Color(0xff1f0757),fontWeight: FontWeight.bold),),),
      //     //cancellationText: ,
