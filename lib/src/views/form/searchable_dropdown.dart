...
class SearchableDropdownField extends StatefulWidget {
  ...
  const SearchableDropdownField({Key? key, required this.question,required this.properties,required this.dropdownProperties}) : super(key: key);
...
class _SearchableDropdownFieldState extends State<SearchableDropdownField> {
  ...
  Widget build(BuildContext context) {
    return DropdownSearch<String>(
      ...
      validator: widget.properties.required ? FormBuilderValidators.required(context,errorText: 'Requerido',) : null,
      onChanged: (value) {
        setState(() {
          widget.properties.answer = value != null ? value.toString().split("|")[0] : null;
        });
      },
