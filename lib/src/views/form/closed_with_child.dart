...
class _ClosedWithChildFieldState extends State<ClosedWithChildField> {
    ...
    if (widget.dropdownProperties.hasChild) {
      for (int i = 0; i < widget.form.sections.length; i++) {
        for (int j = 0; j < widget.form.sections[i].questions.length; j++) {
          if (widget.dropdownProperties ==
              widget.form.sections[i].questions[j].questionId) {
            setState(() {
              principalSection = i;
              principalQuestion = j;
  ...
  void filter() async {
    ...
    widget.form.sections[principalSection].questions[principalQuestion]
        .answers = _answersToShow;
  }
  ...
  void change(compare) async {
    ...
    if (widget.dropdownProperties.hasChild) {
      for (int i = 0; i < widget.form.sections.length; i++) {
        for (int j = 0; j < widget.form.sections[i].questions.length; j++) {
          if (widget.dropdownProperties.principalChild ==
              widget.form.sections[i].questions[j].questionId) {
            setState(() {
              widget.formKey.currentState
                  ?.fields[widget.dropdownProperties.principalChild]
                  ?.setValue(null);
              widget.form.sections[i].questions[j].answer = null;
              widget.form.sections[i].questions[j].answers = _answersToShow;
            });
          }
          if (widget.dropdownProperties.children != null &&
              widget.dropdownProperties.children!
                  .contains(widget.form.sections[i].questions[j].questionId)) {
            setState(() {
              widget.formKey.currentState
                  ?.fields['${widget.form.sections[i].questions[j].questionId}']
                  ?.setValue(null);
              widget.form.sections[i].questions[j].answers = [];
            });
          }
  ...
  Widget build(BuildContext context) {
    return FormBuilderDropdown(
      ...
      items: widget.dropdownProperties.answers
